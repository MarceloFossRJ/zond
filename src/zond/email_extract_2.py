...
agent = create_agent(
    model="gpt-5.4",
    tools=[],
    response_format=ToolStrategy(Details),
    system_prompt="You are a helpful assistant that parses emails. Do not make any field or value up."
)

result = agent.invoke({
    "messages": [{"role": "user", "content": f"Extract contact info from: {email}"}]
})
